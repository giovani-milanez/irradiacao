import { useAuth, User } from '@/utils/use-auth';
...
export default function UsersPage() {
  ...
  const columns: Column<User>[] = [
    {
      key: 'name', label: 'Usuário', sortable: true, render(value, row) {
    ...
    {
      key: 'admin', label: 'Tipo', sortable: true, render(value, row) {
        return (<div>{row.admin ? 'Administrador' : row.member ? 'Membro' : 'Usuário'}</div>)
      },
    },
    {
  ...
  const getData = () => {
    setLoading(true)
    AxiosInstance.get<User[]>('/api/users').then(resp => {
      setData(resp.data)
    }).catch(err => {
  ...
  const onDeleteClick = (selectedId: number) => {
    ...
    toast.error('Nao implementado ainda..')
  }
